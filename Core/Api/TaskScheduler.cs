...
public static class TaskScheduler
{
    ...
    /// <returns></returns>
    public static IEnumerator Coroutine(Action action, ScheduleType scheduleType, int amountToWait, Func<bool> waitCondition = null)
    {
    ...
    private static IEnumerator WaiterCoroutine(ScheduleType scheduleType, int amountToWait)
    {
        switch (scheduleType)
        {
            case ScheduleType.WaitForSeconds:
                var wait = new WaitForSecondsRealtime(1); //todo: see if this has less allocations
                var count = 0;
