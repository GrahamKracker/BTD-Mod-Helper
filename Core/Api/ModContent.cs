...
public abstract class ModContent : IModContent, IComparable<ModContent>
{
    ...
    internal static void LoadModContent(BloonsTD6Mod mod) => mod.Content = mod.GetAssembly()
        .GetValidTypes()
        .Where(CanLoadType)
        .Select(type => CreateInstance(type, mod))
        .Where(content => content != null)
        .OrderBy<ModContent, float>(content => content.RegistrationPriority)
        .ThenBy<ModContent, int>(content => content.Order)
        .SelectMany(Load)
    ...
    protected internal readonly Stack<Action> rollbackActions = new();
